...
describe Avromatic::Model::Builder, 'validation' do
  ...
  shared_examples_for "correct validation" do |mutable:|
    let(:test_class) do
      described_class.model(schema_name: schema_name, mutable: mutable)
    end
    ...
    context "required" do
      ...
      context "array" do
        ...
        let(:test_class) { described_class.model(schema: schema, mutable: mutable) }
        ...
        it "passes validation if all required fields are present" do
          ...
          expect(instance).to be_valid
          expect(instance.send(:prevalidated?)).to eq(!mutable)
        end
      end
      ...
      context "nested records" do
        ...
        let(:test_class) do
          described_class.model(schema: schema.fields_hash['sub'].type, mutable: mutable)
          described_class.model(schema: schema, mutable: mutable)
        end
        ...
        context "doubly nested record" do
          ...
          let(:test_class) do
            described_class.model(schema: schema.fields_hash['sub'].type.fields_hash['sub_sub'].type, mutable: mutable)
            described_class.model(schema: schema.fields_hash['sub'].type, mutable: mutable)
            described_class.model(schema: schema, mutable: mutable)
          end
        ...
        context "array of records" do
          ...
          let(:test_class) do
            described_class.model(schema: schema.fields_hash['ary'].type.items, mutable: mutable)
            described_class.model(schema: schema, mutable: mutable)
          end
        ...
        context "nested arrays of records" do
          ...
          let(:test_class) do
            described_class.model(schema: schema.fields_hash['m'].type.items.items, mutable: mutable)
            described_class.model(schema: schema, mutable: mutable)
          end
        ...
        context "map of records" do
          ...
          let(:test_class) do
            described_class.model(schema: schema.fields_hash['map'].type.values, mutable: mutable)
            described_class.model(schema: schema, mutable: mutable)
          end
        ...
        context "record in a union" do
          ...
          let(:test_class) do
            described_class.model(schema: schema.fields_hash['u'].type.schemas.first, mutable: mutable)
            described_class.model(schema: schema, mutable: mutable)
          end
  ...
  it_behaves_like "correct validation", mutable: true
  it_behaves_like "correct validation", mutable: false

  context "when mixing mutable and immutable models" do
    context "when an immutable model has a mutable model" do
      let(:schema) do
        Avro::Builder.build_schema do
          record :has_record do
            required :sub, :record, type_name: 'sub_type' do
      ...
      let(:test_class) do
        described_class.model(schema: schema.fields_hash['sub'].type, mutable: true)
        described_class.model(schema: schema, mutable: false)
      end

      it "is not prevalidated" do
        instance = test_class.new(sub: { i: 0 })
        expect(instance.send(:prevalidated?)).to eq(false)
        expect(instance).to be_valid
      end
    end

    context "when an immutable model has an array of mutable model" do
      let(:schema) do
        Avro::Builder.build_schema do
          record :sub do
            required :i, :int
          ...
          record :array_of_records do
            required :sub, :array, items: :sub
          end
        end
      end

      let(:test_class) do
        described_class.model(schema: schema.fields_hash['sub'].type.items, mutable: true)
        described_class.model(schema: schema, mutable: false)
      end
    ...
    context "when an immutable model has a map of mutable model" do
      let(:schema) do
        Avro::Builder.build_schema do
          record :sub do
            required :i, :int
          ...
          record :array_of_records do
            required :sub, :map, values: :sub
          end
        end
      end

      let(:test_class) do
        described_class.model(schema: schema.fields_hash['sub'].type.values, mutable: true)
        described_class.model(schema: schema, mutable: false)
      end

      it "is not prevalidated" do
        instance = test_class.new(sub: { a: { i: 0 } })
        expect(instance.send(:prevalidated?)).to eq(false)
        expect(instance).to be_valid
      end
    end
